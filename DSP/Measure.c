...
float Measure_RMS(const float *data, uint32_t len)
{
    if (data == NULL || len == 0u) return 0.0f;
    float s = 0.0f;
    for (uint32_t i = 0; i < len; i++) s += data[i] * data[i];
    return sqrtf(s / (float)len);
}
...
{
    if (res == NULL) return;
    if (mag == NULL || n == 0u || k_peak == 0u || k_peak >= (n / 2u)) {
        /* 边界无法插值，退化为整数 bin 结果 */
        if (res) {
            res->bin = (float)k_peak;
            res->freq = (float)k_peak * fs / (float)n;
            res->amplitude = (mag && k_peak < (n / 2u + 1u)) ? mag[k_peak] : 0.0f;
        }
        return;
    }
                          ...
                          Measure_Peak_t *res)
{
    ...
    for (uint32_t k = 2u; k < half; k++) {
        if (mag[k] > v_max) { v_max = mag[k]; k_max = k; }
    }
...
{
    if (amp == NULL || count < 2u || amp[0] <= 0.0f) {
...
/* 内部：把 [k-half, k+half] 范围内的功率聚合，返回该分量功率，并把这些 bin 标记已用 */
                                  ...
                                  uint8_t *used)
{
    ...
    uint32_t hi = center + leak_bins;
    if (hi >= half_len) hi = half_len - 1u;
                     ...
                     Measure_Quality_t *q)
{
    ...
    if (mag == NULL || n < 8u) return;

    const uint32_t half_len = n / 2u + 1u;      /* 单边谱长度 */

    /* 静态标记数组：支持到 8192 点 FFT(单边 4097)。如需更大请调整。 */
    #define MEASURE_MAX_HALF 513u
    static uint8_t used[MEASURE_MAX_HALF];
    uint32_t mark_len = (half_len <= MEASURE_MAX_HALF) ? half_len : MEASURE_MAX_HALF;
    for (uint32_t i = 0; i < mark_len; i++) used[i] = 0u;

    /* 1) 找基波 bin（跳过直流） */
    uint32_t k0 = 1u;
    float vmax = mag[1];
    for (uint32_t k = 2u; k < (n / 2u); k++) {
        if (mag[k] > vmax) { vmax = mag[k]; k0 = k; }
    }
...
/* 内部：从 start 起找首个穿越 level 的插值位置(采样为单位)，dir>0 上升 dir<0 下降，找不到返回 -1 */
...
void Measure_Waveform(const float *data, uint32_t len, float fs, Measure_Wave_t *res)
{
    if (res == NULL) return;
    /* 清零默认值 */
    res->freq = res->period = res->vpp = res->amplitude = 0.0f;
    res->rise_time = res->fall_time = res->overshoot = 0.0f;
    res->duty = res->form_factor = res->crest_factor = 0.0f;
