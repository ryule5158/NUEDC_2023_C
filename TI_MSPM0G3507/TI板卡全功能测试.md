...
1. 使用外部XDS110-ETP连接MSP-LITO的J3调试接口，按`3V3、GND、SWDIO、SWCLK、UART_RXD、UART_TXD、NRST`信号名逐一对应。
2. XDS110-ETP通过USB连接电脑，并由调试器给MSP-LITO提供3.3 V电源。
3. J3上的PA10/PA11同时作为UART0 TX/RX；使用XDS110回传串口时不要再让另一个USB-TTL驱动这两根线。
4. Keil或UniFlash的目标器件选择MSPM0G3507，调试器选择XDS110。

若改用ULINK2/ULINK-ME，必须先断开XDS110-ETP，再按SWD方式连接J3；不要让两个调试器同时驱动SWD、复位或目标板电源。

### 2.2 TI与BX71 FPGA

高速AD/DA模块与BX71的U3/P3尾端对齐直插，不翻面、不错位，并按模块要求稳定供电。TI与BX71只连接下列五根线：

| TI引脚 | MSP-LITO位置 | BX71 |
|---|---|---|
| PA12 / SPI0_SCLK | J1-30 | P2-31 |
| PA14 / SPI0_PICO（MOSI） | J1-32 | P2-32 |
| PA13 / SPI0_POCI（MISO） | J1-31 | P2-33 |
| PA15 / FPGA_CS | J2-32 | P2-34 |
| GND | 任一GND | P2-30或P2-12 |

两板各自供电，不连接两板的3V3或5V。不得让原STM32同时连接并驱动同一组FPGA SPI线。以调试接口在上、按键在下的元件面观察：J1最下排左/右分别为PA13/PA14，J1倒数第二排右侧为PA12，J2最下排右侧为PA15；不能仅凭杜邦线颜色判断。
...
2. 保持XDS110-ETP与MSP-LITO的J3连接，并用XDS110 USB连接电脑。
