# 使用官方PS7板级配置构建BX71高速AD/DA掉电启动工程。

# 解析脚本位置，得到工程、PS7设计和输出目录。
...
if {![file exists $bd_file]} {
    error "Official BX71 PS7 block design is missing: $bd_file"
}
...
if {![file exists $project_file]} {
    ...
    add_files -norecurse [file join $root_dir "rtl_ps7" "ad9708_zynq_top.v"]
    add_files -norecurse $bd_file
    add_files -fileset constrs_1 -norecurse \
        [file join $root_dir "constraints" "ad9708_gpio1_spi_gpio0.xdc"]
    set_property top ad9708_zynq_top [get_filesets sources_1]
...
# 先综合PS7输出产物，避免Vivado 2018.3并发创建临时目录时发生竞争。
set_property synth_checkpoint_mode Hierarchical [get_files $bd_file]
set ip_run [get_runs -quiet sys_processing_system7_0_0_synth_1]
if {[llength $ip_run] == 1} {
    ...
    if {![string match "*Complete*" $ip_status]} {
        ...
        set ip_status [get_property STATUS $ip_run]
        if {![string match "*Complete*" $ip_status]} {
            error "PS7 IP synthesis failed: $ip_status"
        }
    }
}
...
# 打开已布线设计，确认PS7和AD/DA时钟约束均有效。
open_run impl_1
set report_dir [get_property DIRECTORY [get_runs impl_1]]
if {[llength [get_cells -hier -quiet -filter {REF_NAME == PS7}]] == 0} {
    error "PS7 was optimized out of the implemented design"
}
...
report_timing_summary -delay_type min_max -report_unconstrained \
    -file [file join $report_dir "ad9708_ps7_timing_final.rpt"]
report_drc -file [file join $report_dir "ad9708_ps7_drc_final.rpt"]
report_cdc -details -file [file join $report_dir "ad9708_ps7_cdc_final.rpt"]
# 单独核查ProMax的32 MHz ADC域和50 MHz配置域，避免全局异步复位掩盖真实跨域结果。
report_cdc -details -from [get_clocks clk32] -to [get_clocks clk_50m] \
    -file [file join $report_dir "ad9708_ps7_cdc_adc_to_cfg.rpt"]
report_cdc -details -from [get_clocks clk_50m] -to [get_clocks clk32] \
    -file [file join $report_dir "ad9708_ps7_cdc_cfg_to_adc.rpt"]
report_utilization -hierarchical \
    -file [file join $report_dir "ad9708_ps7_utilization_final.rpt"]
...
set dac_setup_path [get_timing_paths -delay_type max \
    -to [get_ports {ad9708_data_o[*]}] -max_paths 1]
set dac_hold_path [get_timing_paths -delay_type min \
    -to [get_ports {ad9708_data_o[*]}] -max_paths 1]
...
# 每次成功构建都刷新交付目录中的路由后证据，避免遗留旧报告。
set verification_dir [file join $root_dir "output" "verification"]
file mkdir $verification_dir
foreach {source_name evidence_name} {
    ad9708_ps7_timing_final.rpt      timing_final.rpt
    ad9708_ps7_drc_final.rpt         drc_final.rpt
    ad9708_ps7_utilization_final.rpt utilization_final.rpt
    ad9708_ps7_cdc_final.rpt         cdc_global_final.rpt
    ad9708_ps7_cdc_adc_to_cfg.rpt    cdc_adc_to_cfg_final.rpt
    ad9708_ps7_cdc_cfg_to_adc.rpt    cdc_cfg_to_adc_final.rpt
} {
    file copy -force [file join $report_dir $source_name] \
        [file join $verification_dir $evidence_name]
}

# 将含PS7配置的位流复制到统一输出目录。
set bit_file [file join $report_dir "ad9708_zynq_top.bit"]
set output_dir [file join $root_dir "output"]
set output_bit [file join $output_dir "BX71_V4_U3_tail_AD9708_PS7.bit"]
