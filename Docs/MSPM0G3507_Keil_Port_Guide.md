...
`D:\NUEDC\fpga\集成工程\NUEDC_2023_D_MSPM0G3507_TI`
...
| 功能 | MSPM0G3507 | LaunchPad位置 |
|---|---|---|
| UART0 TX/RX | PA10 / PA11 | J21/J22选择XDS或BP |
| ADC0_CH4 | PB25 | 默认从P1/Analog_In经OPA2365缓冲输入 |
| 状态灯 | PB26 / PB27 | 板载RGB红/绿 |
| FPGA SCLK/MOSI/MISO/CS | PA12 / PA14 / PA13 / PA15 | BP32 / J28-7 / BP31 / BP30 |

FPGA SPI0：Controller、Motorola Mode 0、8 bit、MSB first、1 MHz、硬件CS关闭。PA15由软件在整帧前后拉低/拉高；J28为板底未标注排针，PA14必须按缺口和1脚方向定位。

## 4. 外接模块映射

### AD9910

| 信号 | MSPM0G3507 | LaunchPad位置 |
|---|---|---|
| SCK / SDIO | PB20 / PB2 | BP36 / BP9 |
| PWR | PB13 | BP35 |
| DROVER / DRCTL / DRHOLD | PB1 / PA8 / PA16 | BP39 / BP4 / BP29 |
| RESET | PB4 | BP40 |
| PF0 / PF1 / PF2 | PB5 / PA17 / PA18 | J27-1 / BP28 / BP26 |
| OSK / IO_UPDATE / CS | PB7 / PB8 / PB9 | BP14 / BP15 / BP7 |
...
- 共享软件SPI：PB6 SCK（BP12）、PA7 MOSI（J28-6）。
- AD5687 CS：PB22（J27-5，断开J5）。
- DAC8831 CS：PB0（BP11）。
- 两者可静态输出，但不要并行启动两个软件波形任务。

PB0/PB6均从BoosterPack排针引出且无默认板载负载；不使用连接32.768 kHz/40 MHz晶振网络的PA4～PA6。

### ADS8363

- PB17 SCLK（BP18）、PB18 MOSI（BP25）、PB19 MISO（BP23）。
- PB12 CS（BP19）、PB23 RD/CONVST（BP3，J14选择PB23）、PB24 BUSY（BP6，断开J9）。
...
默认构建结果为 Code 12332、RO 396、RW 68、ZI 5988 字节。临时强制引用全部 ProMax 公共API的链接验证为 Code 17540、RO 748、RW 68、ZI 6044 字节，仍远低于128 KB Flash和32 KB SRAM边界；验证后已恢复默认不自动运行的入口。
